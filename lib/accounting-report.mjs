...
export function transactionAccountAllocations(transaction) {
  const rawLineItems = Array.isArray(transaction.lineItems)
    ? transaction.lineItems.filter((item) => Number.isSafeInteger(Number(item?.amountCents)))
    : [];
  const lineItems = reconcileInvoiceLineItemsToSubtotal(
    rawLineItems,
    toCents(transaction.subtotal, transaction.subtotalCents),
    toCents(transaction.total, transaction.totalCents),
  ).lineItems;
...
import { reconcileInvoiceLineItemsToSubtotal } from "./invoice-processing.mjs";
