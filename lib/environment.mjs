...
export const DEMO_PRODUCTION_SEED_EXECUTION_CONFIRMATION = "SEED_DEMO_ONLY";
export const PRODUCTION_CARD_ROSTER_CONFIRMATION = "APPLY_REAL_CARD_ROSTER";
export const PRODUCTION_ACCOUNT_IMPORT_CONFIRMATION = "IMPORT_REAL_ACCOUNTS_FACTURE_THIBEAULT";
...
export function assertSafeDemoProductionSeedExecution({ executionConfirmation, ...target }) {
  assertSafeDemoProductionTarget(target);
  if (executionConfirmation !== DEMO_PRODUCTION_SEED_EXECUTION_CONFIRMATION) {
    throw new Error(`Le seed DEMO production exige la confirmation d'exécution ${DEMO_PRODUCTION_SEED_EXECUTION_CONFIRMATION}.`);
  }
}

export function assertSafeProductionCardRoster({
  projectId,
  adminProjectId,
  appEnvironment,
  publicAppEnvironment,
  ...
  if (!validation.ok || validation.environment !== "production" || projectId !== PRODUCTION_FIREBASE_PROJECT_ID || adminProjectId !== PRODUCTION_FIREBASE_PROJECT_ID) {
    throw new Error(`Configuration du registre de cartes refusée : ${validation.issues.join(" ") || "cible Production exacte requise."}`);
  }
  if (confirmation !== PRODUCTION_CARD_ROSTER_CONFIRMATION) {
    throw new Error(`La configuration du registre de cartes exige la confirmation ${PRODUCTION_CARD_ROSTER_CONFIRMATION}.`);
  }
}

export function assertSafeProductionAccountImport({
  projectId,
  adminProjectId,
  appEnvironment,
  publicAppEnvironment,
  ...
  if (!validation.ok || validation.environment !== "production" || projectId !== PRODUCTION_FIREBASE_PROJECT_ID || adminProjectId !== PRODUCTION_FIREBASE_PROJECT_ID) {
    throw new Error(`Import des comptes réel refusé : ${validation.issues.join(" ") || "cible Production exacte requise."}`);
  }
  if (confirmation !== PRODUCTION_ACCOUNT_IMPORT_CONFIRMATION) {
    throw new Error(`L'import des comptes réels exige la confirmation ${PRODUCTION_ACCOUNT_IMPORT_CONFIRMATION}.`);
  }
}
