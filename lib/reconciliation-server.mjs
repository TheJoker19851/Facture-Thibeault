...
export async function importStatementBatch({ dataConnect, imports, identity, context: suppliedContext = undefined, evidenceWriter = undefined }) {
  ...
  for (const item of imports) {
    ...
    try {
      ...
      // trustedStatementHash is injected only by the authenticated server after
      // it reloads a persisted PDF-analysis artifact. Client-supplied hashes
      // remain ignored by the API route and by the parser above.
      const trustedStatementHash = asId(item.trustedStatementHash);
      const finalized = await finalizeStatementImport({ ...parsed.statement, id: null, statementHash: trustedStatementHash || null }, item.sourceText);
