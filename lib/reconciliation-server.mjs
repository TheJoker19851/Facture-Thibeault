...
export async function loadReconciliationContext(dataConnect, identity) {
  ...
  const [statementsResponse, linesResponse, transactionsResponse, invoicesResponse, aliasesResponse, historiesResponse, matchesResponse, outsideResponse, profilesResponse] = await Promise.all([
    dataConnect.executeQuery("ListCreditCardStatements", undefined, options),
    dataConnect.executeQuery("ListAllCreditCardStatementLines", undefined, options),
    dataConnect.executeQuery("ListExpenseTransactions", undefined, options),
    dataConnect.executeQuery("ListInvoicesForReconciliation", undefined, options),
    dataConnect.executeQuery("ListMerchantAliases", undefined, options),
    dataConnect.executeQuery("ListCreditCardHolderHistories", undefined, options),
    dataConnect.executeQuery("ListReconciliationMatches", undefined, options),
    dataConnect.executeQuery("ListReconciliationOutsideControls", undefined, options),
    dataConnect.executeQuery("ListUserProfiles", undefined, options),
  ]);
  const linesById = statementLinesByStatement(linesResponse.data.creditCardStatementLines ?? []);
  const statements = (statementsResponse.data.creditCardStatements ?? []).map((statement) => mapPersistedStatement(statement, linesById));
  const invoices = invoicesResponse.data.invoices ?? [];
  const invoiceByTransactionId = new Map(invoices.map((invoice) => [asId(invoice.transaction?.id), invoice]));
  const transactions = (transactionsResponse.data.expenseTransactions ?? []).map((transaction) => ({
    ...transaction,
  ...
  return {
    statements,
    lines: linesResponse.data.creditCardStatementLines ?? [],
    transactions,
    invoices,
    aliases: aliasesResponse.data.merchantAliases ?? [],
    aliasRules: normalizeMerchantAliasRows(aliasesResponse.data.merchantAliases ?? []),
    histories: historiesResponse.data.creditCardHolderHistories ?? [],
    matches: matchesResponse.data.reconciliationMatches ?? [],
    outsideControls: outsideResponse.data.reconciliationOutsideControls ?? [],
    profiles: profilesResponse.data.userProfiles ?? [],
  };
}
...
export async function importStatementBatch({ dataConnect, imports, identity, context: suppliedContext = undefined }) {
  ...
  for (const item of imports) {
    ...
    try {
      ...
      await dataConnect.executeMutation("PersistCreditCardStatementImport", {
        ...
        statement: {
          ...
          creditCardStatementLines_on_statement: payload.lines.map((line) => Object.fromEntries(Object.entries(line).filter(([key]) => key !== "statement"))),
        },
...
export async function persistManualMatch({ dataConnect, identity, context, statementId, lineId, transactionId, invoiceId, previousMatch, action = AUDIT_ACTIONS.STATEMENT_MATCH_CONFIRMED, matchScore = 100, matchMethod = "MANUAL", reason = "Jumelage manuel confirmé par Kim." }) {
  if (!ROLE_ALLOWLIST.has(identity?.role)) throw new Error("Le rôle KIM ou ADMIN est requis.");
...
export async function resolveOutsideControl({ dataConnect, identity, context, controlId, resolutionNote }) {
  ...
  await dataConnect.executeMutation("ResolveReconciliationOutsideControl", {
    ...
    resolutionNote: resolutionNote || "Cas hors relevé résolu par Kim.",
...
export async function persistLineStatus({ dataConnect, identity, statementId, lineId, status, previousResult }) {
  ...
  await dataConnect.executeMutation("PersistReconciliationLineStatus", {
    ...
    statementLineId: lineId,
    status,
    reason: status === RECONCILIATION_STATUSES.IGNORED ? "Ligne ignorée manuellement par Kim." : status === RECONCILIATION_STATUSES.MISSING_INVOICE ? "Facture manquante confirmée manuellement." : "Ligne rouverte pour vérification.",
    details: auditDetails(details),
    auditEventId: auditEventId(lineId, action),
    actorUid: identity.uid,
    actorRole: identity.role,
