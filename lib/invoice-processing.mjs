...
export function classifyInvoice(input, skuReferences = [], accounts = []) {
  ...
  const accountMatches = accounts.filter((account) => normalized(account.label) === normalized(category));
  const accountCandidates = [...new Set(accountMatches.map((account) => account.code))];
