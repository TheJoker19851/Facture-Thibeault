...
import { canadianTireSkusMatch, isCanadianTireMerchant } from "./canadian-tire-sku.mjs";
...
const LEGACY_CATEGORY_REVIEW_NOTE = "Catégorie rapprochée du référentiel; une validation manuelle reste requise.";
const LEGACY_SKU_REVIEW_NOTE = "SKU et fournisseur reconnus; une validation manuelle reste requise.";

export function classificationNoteForDisplay({ note, accountCode, category, accountingStatus } = {}) {
  const value = String(note ?? "").trim();
  if (!value || accountingStatus !== "POSTED") return value;

  const accountReference = String(accountCode ?? "").trim();
  const categoryReference = String(category ?? "").trim();
  const accountLabel = accountReference ? `Compte ${accountReference}` : "Compte comptable";
  const categoryNote = categoryReference
    ? `${accountLabel} attribué automatiquement par correspondance exacte avec la catégorie ${categoryReference}.`
    : `${accountLabel} attribué automatiquement par correspondance exacte avec le référentiel.`;
  const skuNote = `${accountLabel} attribué automatiquement à partir d’une référence SKU validée.`;

  return value
    .replace(LEGACY_CATEGORY_REVIEW_NOTE, categoryNote)
    .replace(LEGACY_SKU_REVIEW_NOTE, skuNote);
}
 ...
 * reconcile to the invoice subtotal within one cent.
 */
export function validateInvoiceLineItemsForCommit(lineItems, subtotalCents) {
...
export function validateInvoiceExtraction(candidate) {
  ...
  const lineItems = normalizeInvoiceLineItems(rawLineItems);
 ...
 * merchant match. An exact active expense-category match resolves the account.
 * When the AI cannot name a category, the active Divers account is only a
 * proposal; it must never auto-post without Kim confirming it.
 *
...
export function classifyInvoice(input, skuReferences = [], accounts = []) {
  ...
  const skuMatches = sku && vendor
    ? skuReferences.filter((reference) => {
      ...
      const canadianTireMatch = isCanadianTireMerchant(input?.vendor) && isCanadianTireMerchant(reference.merchant);
      const merchantMatches = canadianTireMatch || (Boolean(merchant) && (vendor === merchant || vendor.includes(merchant) || merchant.includes(vendor)));
      const skuMatchesReference = canadianTireMatch
        ? canadianTireSkusMatch(input?.sku, reference.sku)
        : normalizedSku(reference.sku) === sku;
      return merchantMatches && skuMatchesReference;
    })
    : [];
  ...
  if (sku && vendor) {
    ...
    if (skuMatch?.accountCode) {
      const validated = skuMatch.status === "Validé" || skuMatch.status === "VALIDATED" || skuMatch.status === "AUTO_VERIFIED";
      const productLabel = typeof skuMatch.productLabel === "string" ? skuMatch.productLabel.trim() : "";
      return {
        ...
        note: validated
          ? `Compte ${skuMatch.accountCode} attribué automatiquement à partir d’une référence SKU validée${productLabel ? ` (${productLabel})` : ""}.`
          : "SKU reconnu, mais la référence doit encore être confirmée.",
  ...
  if (accountMatch) {
    return {
      ...
      note: suppliedCategory
        ? `Compte ${accountMatch.number ?? accountMatch.code} attribué automatiquement par correspondance exacte avec la catégorie ${accountMatch.label}.`
