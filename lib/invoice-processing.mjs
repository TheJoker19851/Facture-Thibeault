 ...
 * Amounts must be integers in CAD cents. Arithmetic discrepancies are kept as
 * warnings so the extracted evidence reaches manual review; the decision
 * engine independently blocks posting until totals reconcile.
 *
 * @param {unknown} candidate
 * @returns {{ok: true, value: object, errors: [], warnings: string[]} | {ok: false, value: null, errors: string[], warnings: string[]}}
 */
export function validateInvoiceExtraction(candidate) {
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) {
    return { ok: false, value: null, errors: ["La réponse OCR n'est pas un objet."], warnings: [] };
  }
  ...
  if (Number.isSafeInteger(subtotalCents) && Number.isSafeInteger(tpsCents) && Number.isSafeInteger(tvqCents) && Number.isSafeInteger(totalCents)) {
    if (Math.abs(subtotalCents + tpsCents + tvqCents - totalCents) > 1) {
      warnings.push("Le total ne correspond pas au sous-total et aux taxes.");
    }
  }
  ...
  if (errors.length) return { ok: false, value: null, errors, warnings };
  return {
    ok: true,
    errors: [],
    warnings,
    value: {
      ...
      isRestaurant: input.isRestaurant === true,
 ...
 * merchant match. An exact match to one active expense-account category is
 * resolved deterministically. When the AI cannot name a category, the active
 * Divers account remains only a proposal and cannot auto-post.
 *
  ...
  if (sku && vendor) {
    ...
    if (skuMatch?.accountCode) {
      ...
      return {
        ...
        note: validated ? "SKU et fournisseur reconnus dans le référentiel actif." : "SKU reconnu, mais la référence doit encore être confirmée.",
  ...
  if (accountMatch) {
    return {
      ...
      note: suppliedCategory
        ? "Catégorie rapprochée du référentiel actif."
...
const RESTAURANT_CATEGORY_MARKERS = [
  "restaurant",
  "restauration",
  "repas",
  "fast food",
...
function isRestaurantCategory(value) {
  ...
  return Boolean(category) && RESTAURANT_CATEGORY_MARKERS.some((marker) => category.includes(marker));
}

/**
 * Apply the cardholder-specific restaurant policy after extraction and before
 * the deterministic decision. Restaurant receipts for Keven Tremblay go to
 * Divers; restaurant receipts for every other identified holder go to Voyage
 * et pension. Every line is deliberately assigned to the same target account.
 *
 * @param {{
 ...
 *   accounts?: Array<{ number?: string, code?: string, status?: string, type?: string, label?: string }>
 * }} options
 */
export function applyCardholderRestaurantPolicy({ holderName, isRestaurant = false, invoiceCategory, lineItems = [], accounts = [] } = {}) {
  const normalizedLines = normalizeInvoiceLineItems(lineItems);
  const holder = normalized(holderName);
  const restaurantDetected = isRestaurant === true || isRestaurantCategory(invoiceCategory);
  if (!holder || !restaurantDetected) {
    return { applied: false, accountCode: null, category: null, note: null, lineItems: normalizedLines };
  }

  const targetAccountCode = holder === "keven tremblay" ? "33526" : "34016";
  const targetAccount = accounts.find((account) => {
    const accountCode = account.number ?? account.code;
    const active = !account.status || account.status === "ACTIVE" || account.status === "Actif";
    const expenseType = !account.type || account.type === "EXPENSE" || account.type === "Dépense";
    return accountCode === targetAccountCode && active && expenseType;
  });
  if (!targetAccount) {
    return {
      applied: false,
      accountCode: null,
      category: null,
      note: `La règle restaurant n’a pas été appliquée parce que le compte ${targetAccountCode} n’est pas actif.`,
      lineItems: normalizedLines,
    };
  }

  const accountCode = targetAccount.number ?? targetAccount.code;
  const note = holder === "keven tremblay"
    ? "Règle restaurant : toutes les lignes de Keven Tremblay sont classées dans Divers."
    : "Règle restaurant : toutes les lignes des autres détenteurs sont classées dans Voyage et pension.";
  return {
    applied: true,
    accountCode,
    category: targetAccount.label,
    note,
    lineItems: normalizedLines.map((item) => ({
      ...item,
      category: targetAccount.label,
      accountCode,
      classificationSource: "CARDHOLDER_RESTAURANT_POLICY",
