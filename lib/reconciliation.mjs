...
export function validateStatementPdfExtraction(value) {
  ...
  const globalCardLastFour = asText(value.cardLastFour).replace(/\D/g, "").slice(-4);
  boundedSourceLines.forEach((line, index) => {
    const cardLastFour = asText(isObject(line) ? line.cardLastFour : "").replace(/\D/g, "").slice(-4) || globalCardLastFour;
    if (!/^\d{4}$/.test(cardLastFour)) lineErrors.push(`lines[${index}].cardLastFour doit contenir les quatre derniers chiffres de la carte.`);
  });
  ...
  return {
    extraction: {
      ...
      lines: lines.map(({ transactionDate, postedDate, merchantRaw, amountCents, externalReference }, index) => ({
        sequence: index + 1,
        cardLastFour: asText(boundedSourceLines[index]?.cardLastFour).replace(/\D/g, "").slice(-4) || globalCardLastFour,
...
export function matchStatementPdfCard(extraction, cards = []) {
  ...
  const lastFour = asText(extraction?.cardLastFour).replace(/\D/g, "").slice(-4);
  ...
  if (candidates.length > 1 && holderKey) {
    const holderMatches = candidates.filter((card) => merchantKey(
      isObject(card.holder) ? card.holder.displayName : card.holder ?? card.holderName,
    ) === holderKey);
    if (holderMatches.length) candidates = holderMatches;
  }
