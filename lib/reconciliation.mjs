...
export function validateStatementPdfExtraction(value) {
  ...
  const lineErrors = [];
  const lines = sourceLines.slice(0, MAX_PDF_STATEMENT_LINES).map((line, index) => normalizeLine(line, index, lineErrors)).filter(Boolean);
  ...
  return {
    extraction: {
      cardLastFour: asText(value.cardLastFour).replace(/\D/g, "").slice(-4),
