...
export const MAX_PDF_STATEMENT_LINES = 2_000;
...
export function normalizeMerchant(value, aliases = DEFAULT_MERCHANT_ALIASES) {
  ...
  const alias = aliases.find((candidate) => {
    if (candidate?.active === false || candidate?.status === "INACTIVE") return false;
    const keys = [
      ...(Array.isArray(candidate?.keys) ? candidate.keys : []),
      candidate?.merchantRawKey,
      candidate?.merchantNormalized,
    ].filter(Boolean);
    return keys.some((candidateKey) => merchantKey(candidateKey) === key);
  });
  return alias?.normalized ?? alias?.merchantCanonical ?? alias?.merchantNormalized ?? raw;
}
...
export function normalizeMerchantAliasRows(rows = []) {
  return rows
    .filter((row) => row && row.active !== false && row.status !== "INACTIVE")
    .map((row) => ({
      normalized: asText(row.merchantCanonical ?? row.merchantNormalized ?? row.normalized),
      merchantCanonical: asText(row.merchantCanonical ?? row.merchantNormalized ?? row.normalized),
      merchantRawKey: asText(row.merchantRawKey),
      keys: [row.merchantRawKey, ...(Array.isArray(row.keys) ? row.keys : [])].filter(Boolean),
      active: true,
      source: row.source ?? "PERSISTED",
    }))
    .filter((row) => row.normalized && row.keys.length);
}

export function applyMerchantAliases(statement, aliases = DEFAULT_MERCHANT_ALIASES) {
  return {
    ...statement,
    lines: statement.lines.map((line) => ({
      ...line,
      merchantNormalized: normalizeMerchant(line.merchantRaw, aliases),
    })),
  };
}
...
function normalizeLine(value, index, errors) {
  ...
  const suppliedSequence = value.sequence == null ? index + 1 : Number(value.sequence);
  if (!Number.isInteger(suppliedSequence) || suppliedSequence !== index + 1) errors.push(`lines[${index}].sequence doit être ${index + 1} et suivre l’ordre du fichier.`);
...
/**
 * Validates and normalizes the structured result returned by the statement
 * PDF model. This deliberately reuses the same line rules as JSON/CSV imports
 * so AI output cannot bypass the accounting contract.
 */
export function validateStatementPdfExtraction(value) {
  const errors = [];
  if (!isObject(value)) return { extraction: null, errors: ["La réponse IA du relevé est invalide."] };
  const periodStart = asText(value.periodStart);
  const periodEnd = asText(value.periodEnd);
  if (!isValidDate(periodStart)) errors.push("La date de début du relevé est absente ou invalide.");
  if (!isValidDate(periodEnd)) errors.push("La date de fin du relevé est absente ou invalide.");
  if (periodStart && periodEnd && periodStart > periodEnd) errors.push("La période détectée du relevé est inversée.");
  const sourceLines = Array.isArray(value.lines) ? value.lines : [];
  if (!sourceLines.length) errors.push("Aucune transaction n’a été détectée dans le PDF.");
  if (sourceLines.length > MAX_PDF_STATEMENT_LINES) errors.push(`Le PDF dépasse ${MAX_PDF_STATEMENT_LINES} transactions.`);
  const lineErrors = [];
  const boundedSourceLines = sourceLines.slice(0, MAX_PDF_STATEMENT_LINES);
  const lines = boundedSourceLines.map((line, index) => normalizeLine(line, index, lineErrors)).filter(Boolean);
  const normalizeCardLastFour = (candidate) => {
    const digits = asText(candidate).replace(/\D/g, "");
    return digits.length === 4 ? digits : "";
  };
  const globalCardLastFour = normalizeCardLastFour(value.cardLastFour);
  boundedSourceLines.forEach((line, index) => {
    const cardLastFour = normalizeCardLastFour(isObject(line) ? line.cardLastFour : "") || globalCardLastFour;
    const holderName = asText(isObject(line) ? line.holderName : "") || asText(value.holderName);
    if (!cardLastFour && !holderName) lineErrors.push(`lines[${index}] doit identifier la carte par ses quatre derniers chiffres ou par son titulaire visible.`);
  });
  errors.push(...lineErrors);
  const confidence = Number(value.confidence);
  if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1) errors.push("La confiance de l’analyse IA est invalide.");
  if (errors.length) return { extraction: null, errors };
  return {
    extraction: {
      cardLastFour: globalCardLastFour,
      holderName: asText(value.holderName) || null,
      periodStart,
      periodEnd,
      confidence,
      notes: asText(value.notes),
      lines: lines.map(({ transactionDate, postedDate, merchantRaw, amountCents, externalReference }, index) => ({
        sequence: index + 1,
        cardLastFour: normalizeCardLastFour(boundedSourceLines[index]?.cardLastFour) || globalCardLastFour,
        holderName: asText(boundedSourceLines[index]?.holderName) || asText(value.holderName) || null,
        transactionDate,
        postedDate,
        merchantRaw,
        amountCents,
...
export function matchStatementPdfCard(extraction, cards = []) {
  const activeCards = cards.filter((card) => card && card.status !== "Inactif" && card.status !== "INACTIVE");
  const extractedDigits = asText(extraction?.cardLastFour).replace(/\D/g, "");
  const lastFour = extractedDigits.length === 4 ? extractedDigits : "";
  const holderKey = merchantKey(extraction?.holderName);
  let candidates = lastFour ? activeCards.filter((card) => asText(card.lastFour).replace(/\D/g, "").slice(-4) === lastFour) : activeCards;
  const holderMatches = holderKey ? activeCards.filter((card) => merchantKey(
    isObject(card.holder) ? card.holder.displayName : card.holder ?? card.holderName,
  ) === holderKey) : [];
  if (candidates.length > 1 && holderKey) {
    const matchingCandidates = candidates.filter((card) => holderMatches.includes(card));
    if (matchingCandidates.length) candidates = matchingCandidates;
  }
  if (candidates.length === 1) {
    const card = candidates[0];
    return { cardId: asText(card.id), card, error: null };
  }
  if (candidates.length === 0 && holderMatches.length === 1) {
    const card = holderMatches[0];
    return { cardId: asText(card.id), card, error: null };
  }
  if (!lastFour) return { cardId: null, card: null, error: "L’IA n’a pas trouvé les quatre derniers chiffres de la carte." };
  if (!candidates.length) return { cardId: null, card: null, error: `La carte •••• ${lastFour} détectée dans le PDF n’est pas active.` };
  return { cardId: null, card: null, error: `Plusieurs cartes actives terminent par ${lastFour}; le titulaire doit être vérifié.` };
}

/**
 * Splits a consolidated/master statement into one accounting statement per
 * active card. Card identity is resolved on every line so the master account
 * number can never be mistaken for an employee card.
 */
export function splitStatementPdfExtractionByCard(extraction, cards = []) {
  const groups = new Map();
  const errors = [];
  for (const [index, line] of (extraction?.lines ?? []).entries()) {
    const matched = matchStatementPdfCard({
      cardLastFour: line.cardLastFour || extraction?.cardLastFour,
      holderName: line.holderName || extraction?.holderName,
    }, cards);
    if (!matched.cardId) {
      errors.push(`Ligne ${index + 1} : ${matched.error ?? "la carte n’a pas pu être identifiée."}`);
      continue;
    }
    const current = groups.get(matched.cardId) ?? { card: matched.card, lines: [] };
    current.lines.push(line);
    groups.set(matched.cardId, current);
  }
  if (errors.length) return { statements: [], errors };
  return {
    statements: Array.from(groups, ([cardId, group]) => ({
      cardId,
      card: group.card,
      extraction: {
        ...extraction,
        cardLastFour: asText(group.card?.lastFour).replace(/\D/g, "").slice(-4),
        holderName: asText(isObject(group.card?.holder) ? group.card.holder.displayName : group.card?.holder ?? group.card?.holderName) || null,
        lines: group.lines.map((line, index) => ({ ...line, sequence: index + 1 })),
      },
    })),
    errors: [],
  };
...
export function statementSourceFromPdfExtraction(extraction, metadata) {
  return JSON.stringify({
    cardId: asText(metadata?.cardId),
    periodStart: asText(extraction?.periodStart),
    periodEnd: asText(extraction?.periodEnd),
    holderNameSnapshot: asText(extraction?.holderName) || null,
...
export function reconcileStatement(statement, transactions, aliases = DEFAULT_MERCHANT_ALIASES) {
  ...
  const candidateIds = new Set(lineResults.flatMap((result) => result.candidates.map((candidate) => candidate.transaction.id)));
  const outsideTransactions = periodTransactions
    .filter((transaction) => !matchedIds.has(transaction.id) && !candidateIds.has(transaction.id))
...
export function setLineReconciliationStatus(reconciliation, lineId, status, actor = {}) {
  ...
  const result = reconciliation.lineResults.find((candidate) => candidate.line.id === lineId);
  if (!result) throw new Error("Ligne de relevé introuvable.");
  const updated = { ...result, status, match: null, reason: status === RECONCILIATION_STATUSES.IGNORED ? "Ligne ignorée manuellement." : status === RECONCILIATION_STATUSES.MISSING_INVOICE ? "Facture manquante confirmée manuellement." : "Ligne rouverte pour vérification." };
