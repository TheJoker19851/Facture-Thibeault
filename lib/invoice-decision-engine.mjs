...
export function decideInvoice({
  ...
  if (Object.prototype.hasOwnProperty.call(value, "lineItems")) {
    ...
    unresolvedLines = classifiedLines.filter((item) => !item?.accountCode || !["RESOLVED", "CONFIRMED"].includes(item?.classificationStatus));
    addCheck(checks, exceptions, {
    ...
    lineItemAccounts = [...new Set(classifiedLines.map((item) => item?.accountCode).filter(Boolean))];
    const splitResolved = lineItemAccounts.length <= 1 || unresolvedLines.length === 0;
    addCheck(checks, exceptions, {
      code: "LINE_ITEM_ACCOUNT_SPLIT",
      passed: splitResolved,
      message: lineItemAccounts.length <= 1
        ? "Les lignes utilisent un compte principal unique."
        : splitResolved
          ? "La ventilation sur plusieurs comptes est complète et peut être comptabilisée automatiquement."
          : "La ventilation sur plusieurs comptes contient une ligne non résolue.",
  ...
  if (sku && classificationValue.skuState === "UNKNOWN") {
    ...
    addCheck(checks, exceptions, {
      ...
      passed: true,
      message: sku
        ? classificationValue.skuState === "POLICY_OVERRIDDEN"
          ? "Le SKU n’est pas requis par la règle restaurant."
          : "Le SKU est résolu."
        : "Aucun SKU n’est requis par défaut pour cette V1.",
    });
  }

  const summaryAccountResolved = classificationValue.resolution === "RESOLVED" && Boolean(classificationValue.accountCode);
  const lineAccountsResolved = lineItems.length > 0 && unresolvedLines.length === 0 && lineItemAccounts.length > 0;
  const accountResolved = summaryAccountResolved || lineAccountsResolved;
  const resolvedAccountCode = lineAccountsResolved
    ? lineItemAccounts.length === 1 ? lineItemAccounts[0] : null
    : summaryAccountResolved ? classificationValue.accountCode : null;
  const accountProposed = !lineAccountsResolved && classificationValue.resolution === "PROPOSED" && Boolean(classificationValue.accountCode);
  const accountAmbiguous = !lineAccountsResolved && classificationValue.resolution === "AMBIGUOUS";
  addCheck(checks, exceptions, {
    code: "ACCOUNT_RESOLUTION",
    passed: accountResolved,
    message: accountResolved
      ? lineItemAccounts.length > 1
        ? "Tous les comptes de la ventilation des lignes sont résolus."
  ...
  const projectRequired = context.requireProject !== false;
  addCheck(checks, exceptions, {
    code: "PROJECT_RESOLUTION",
    passed: !projectRequired || projectResolved || projectAllowedMissing,
    message: !projectRequired ? "Le projet est temporairement facultatif et pourra être ajouté manuellement." : projectResolved ? "Le projet extrait existe et est actif." : projectAllowedMissing ? "L’absence de projet est autorisée par le contexte métier." : "Le projet est absent, inconnu ou inactif.",
