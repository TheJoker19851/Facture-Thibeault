...
export function buildAccountingReportExcelXml({ period, transactions = [], accounts = [] }) {
  ...
  const groupRows = (totals) => Array.from(totals.entries()).map(([key, value]) => [key, dollars(value.subtotal), dollars(value.tps), dollars(value.tvq), dollars(value.total)]);
...
${worksheet("PAR_COMPTE", ["Compte", "Avant taxes", "TPS", "TVQ", "Total"], accountRows.map((values) => [values[0], values[2], values[3], values[4], values[5]]))}
${worksheet("PAR_PROJET", ["Projet", "Avant taxes", "TPS", "TVQ", "Total"], groupRows(projectTotals))}
${worksheet("PAR_PERSONNE", ["Titulaire", "Avant taxes", "TPS", "TVQ", "Total"], groupRows(personTotals))}
${worksheet("PAR_CARTE", ["Carte", "Avant taxes", "TPS", "TVQ", "Total"], groupRows(cardTotals))}
</Workbook>`;
}
