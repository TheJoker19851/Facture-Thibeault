...
const ACCOUNT_LABEL_ROW_HEIGHT = 36;
...
function row(values, { styles = {}, formulas = {}, types = {}, height } = {}) {
  const heightAttributes = height == null ? "" : ` ss:Height="${height}" ss:AutoFitHeight="0"`;
  return `<Row${heightAttributes}>${values.map((value, index) => cell(value, { style: styles[index] ?? "", formula: formulas[index] ?? "", type: types[index] })).join("")}</Row>`;
}
...
function templateRows(report) {
  ...
  rows.push({ values: [null, null, null, null, null, "TPS", "TVQ", null, ...accountLabels], styles: { 5: "Header", 6: "Header", ...Object.fromEntries(accountLabels.map((value, index) => value == null ? [] : [index + 8, "HeaderAccountLabel"])) }, height: ACCOUNT_LABEL_ROW_HEIGHT });
...
export function buildAccountingReportExcelXml(input = {}) {
  ...
  const worksheetRows = rows.map((entry) => typeof entry === "string" ? entry : row(entry.values, { styles: entry.styles, formulas: entry.formulas, types: entry.types, height: entry.height })).join("");
...
function xlsxWorksheetXml(report) {
  ...
  const worksheetRows = templateRows(report).map((entry, index) => {
    ...
    const height = entry.height == null ? "" : ' ht="' + entry.height + '" customHeight="1"';
