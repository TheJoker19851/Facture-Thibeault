...
test("le chemin AUTO_APPROVED ne dépend pas d'un projet deviné par l'IA", async () => {
  ...
  assert.match(branch, /statementPeriodId: null,\s+(?:\/\/[^\n]*\s+)*projectId: null,/);
  assert.doesNotMatch(branch, /projectId: extraction\.projectId/);
});
